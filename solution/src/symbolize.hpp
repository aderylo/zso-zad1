                                             ...
                                             utils::Relocation          org_rel )
{
    ...
    if ( obj_class == utils::TEXT ) {
        ...
        return utils::Relocation( org_obj_addr - referenced_sym.value, referenced_sym.__symtab_idx,
                                  org_rel.type, 0x0 );
    }
    ...
    if ( obj_class == utils::DATA ) {
        if ( existing_sym.empty() ) { // add section (with data) and symbol pointing to it
            ...
            new_data_sec->set_data( org_mem_layout.data.data + offset_into_data, org_obj_size );

            std::cout << "Section name:" << new_data_sec->get_name() << " :\n";
            std::cout << org_mem_layout.data.data << std::endl;
            std::cout << offset_into_data << std::endl;
            std::cout << org_obj_size << std ::endl;
                                                ...
                                                std::vector<utils::Relocation> org_reltab_view )
{
    ...
    for ( auto org_rel : org_reltab_view ) {
        ...
        utils::Relocation new_rel = recreate_relocation_entry(
            new_file, new_symtab_acc, new_strtab_acc, org_mem_layout, org_symtab_view, org_rel );
                           ...
                           section*& new_strtab_sec )
{
    ...
    for ( section* new_fn_sec : new_fn_sections ) {
        auto new_reltab_view =
            recreate_reltab( new_file, new_symtab_acc, new_strtab_acc, org_mem_layout, new_fn_sec,
                             org_symtab_view, org_reltab_view );
